...
@Component({
  selector: 'app-business-report-loan-paidloan',
  templateUrl: './business-report-loan-paidloan.component.html',
  styleUrls: ['./business-report-loan-paidloan.component.scss'],
})
export class BusinessReportLoanPaidloanComponent implements OnInit {
...
console.log(dataSetgroupBy);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'وام های پرداخت شده':' وام های پرداخت شده سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
	...
	async getData(	filtered_business_id:string=this.businessId,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/paidLoanList', {
