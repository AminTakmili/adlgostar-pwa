...
@Component({
  selector: 'app-business-report-loan-unpaidloan',
  templateUrl: './business-report-loan-unpaidloan.component.html',
  styleUrls: ['./business-report-loan-unpaidloan.component.scss'],
})
export class BusinessReportLoanUnpaidloanComponent implements OnInit {
...
console.log(dataSetgroupBy);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'وام های پرداخت نشده':' وام های پرداخت نشده سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع وام های پرداخت شده هرماه را به تفکیک میتوانید مشاهده کنید ' 
        ...
        `<table dir="rtl"><tr><td  style="font-size:10px ">{series.name}</td></tr>`+
			...
			series: [
				{
					type: 'column',
					name: 'مجموع وام های پرداخت نشده',
					colorByPoint: true,
					data: dataColumn,
					showInLegend: false,
	...
	async getData(	filtered_business_id:string=this.businessId,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/unPaidLoanList', {
