...
@Component({
  selector: 'app-business-report-loan-workinghourcount',
  templateUrl: './business-report-loan-workinghourcount.component.html',
  styleUrls: ['./business-report-loan-workinghourcount.component.scss'],
})
export class BusinessReportLoanWorkinghourcountComponent implements OnInit {

  pageTitle: string = 'گزارشات زمان حضور در کارگاه';
	...
	setcolumnChart(dataSet: reportLeave[]) {
		...
		this.columnChartOptions = {
			...
			title: {
				text:
					Object.keys(dataSetgroupBy).length != 1
						? 'زمان حضور در کارگاه'
						: ' زمان حضور در کارگاه سال ' + Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text: ' مجموع زمان حضور در کارگاه به تفکیک هر ماه را میتوانید مشاهده کنید ',
			},
			tooltip: {
				// headerFormat:,
				pointFormat:
			...
			series: [
				{
					type: 'column',
					name: 'مجموع زمان حضور در کارگاه',
					colorByPoint: true,
					data: dataColumn,
					showInLegend: false,
	...
	async getData(
		filtered_business_id: string = this.id,
		...
		// }
		this.global
			.httpPost('report/payroll/workingHourCountLis', {
