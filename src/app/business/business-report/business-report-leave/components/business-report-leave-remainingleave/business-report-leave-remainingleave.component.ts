...
@Component({
  selector: 'app-business-report-leave-remainingleave',
  templateUrl: './business-report-leave-remainingleave.component.html',
  styleUrls: ['./business-report-leave-remainingleave.component.scss'],
})
export class BusinessReportLeaveRemainingleaveComponent implements OnInit,OnChanges {
...
console.log(dataSetgroupBy);
    ...
    console.log(dataCategories);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'مرخصی باقی مانده':' مرخصی های باقیمانده سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع مرخصی های باقی مانده هرماه را به تفکیک میتوانید مشاهده کنید ' 
        ...
        `<table dir="rtl"><tr><td  style="font-size:10px ">{series.name}</td></tr>`+
			...
			series: [
				{
					type: 'column',
					name: 'مجموع مرخصی های باقیمانده',
					colorByPoint: true,
					data: dataColumn,
					showInLegend: false,
