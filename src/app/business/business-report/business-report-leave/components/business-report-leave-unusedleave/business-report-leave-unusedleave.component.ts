...
@Component({
  selector: 'app-business-report-leave-unusedleave',
  templateUrl: './business-report-leave-unusedleave.component.html',
  styleUrls: ['./business-report-leave-unusedleave.component.scss'],
})
export class BusinessReportLeaveUnusedleaveComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    console.log(dataCategories);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'مرخصی استفاده نشده':' مرخصی های استفاده نشده سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع مرخصی های استفاده نشده هرماه را به تفکیک میتوانید مشاهده کنید ' 
      ...
      tooltip: {
        ...
        `<table dir="rtl"><tr><td  style="font-size:10px ">{series.name}</td></tr>`+
			...
			series: [
				{
					type: 'column',
					name: 'مجموع مرخصی های استفاده نشده',
					colorByPoint: true,
					data: dataColumn,
					showInLegend: false,
