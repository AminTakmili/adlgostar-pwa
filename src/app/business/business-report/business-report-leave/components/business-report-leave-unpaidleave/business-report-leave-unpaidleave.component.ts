...
@Component({
  selector: 'app-business-report-leave-unpaidleave',
  templateUrl: './business-report-leave-unpaidleave.component.html',
  styleUrls: ['./business-report-leave-unpaidleave.component.scss'],
})
export class BusinessReportLeaveUnpaidleaveComponent implements OnInit {
...
console.log(dataSetgroupBy);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'مرخصی های پرداخت نشده':' مرخصی های پرداخت نشده سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع مرخصی های پرداخت نشده هرماه را به تفکیک میتوانید مشاهده کنید ' 
        ...
        `<table dir="rtl"><tr><td  style="font-size:10px ">{series.name}</td></tr>`+
			...
			series: [
				{
					type: 'column',
					name: 'مجموع مرخصی های پرداخت نشده',
					colorByPoint: true,
					data: dataColumn,
					showInLegend: false,
	...
	async getData(	filtered_business_id:string=this.businessId,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/unPaidLeaveList', {
