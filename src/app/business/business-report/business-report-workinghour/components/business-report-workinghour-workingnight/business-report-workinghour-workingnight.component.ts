...
import {  reportWorkingNight } from 'src/app/core/models/report.model';
...
@Component({
  selector: 'app-business-report-workinghour-workingnight',
  templateUrl: './business-report-workinghour-workingnight.component.html',
  styleUrls: ['./business-report-workinghour-workingnight.component.scss'],
})
export class BusinessReportWorkinghourWorkingnightComponent implements OnInit {
 
  @Input('businessId')businessId:string
	columnHighcharts: typeof Highcharts = Highcharts;
	plotShow: boolean = false;
	data: reportWorkingNight[];
  ...
  setcolumnChart(dataSet: reportWorkingNight[]) {
		if (this.plotShow) {
			this.plotShow=false
		}
		console.log(dataSet);
...
console.log(dataSetgroupBy);
		...
		for (const year in dataSetgroupBy) {
			for (let index = startMonth; index <= endMonth; index++) {
				...
				if (
					...
					fackData.push(
						new reportWorkingNight().deserialize({
							business_name: '',
							year,
							month: index,
		...
		const dataworkingOverTimeHourCount = fackData.map((item: reportWorkingNight) => {
			return Math.round(item.working_night_hour_count*10)/10 ;
		});
		const dataworkingOverTimePrice = fackData.map((item: reportWorkingNight) => {
			return Math.round(item.working_night_price*10)/10 ;
		});
		const dataCategories = fackData.map((item: reportWorkingNight) => {
    ...
    console.log(dataCategories);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'شب کاری':' شب کاری سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع شب کاری به تفکیک هر ماه را میتوانید مشاهده کنید ' 
	...
	async getData(	filtered_business_id:string=this.businessId,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/workingNightList', {
        filtered_business_id ,
			...
			.subscribe(
				async (res: any) => {
					...
					this.loading=false
					console.log(res);
					this.data = res.list.map((item: reportWorkingNight) => {
						return new reportWorkingNight().deserialize(item);
					});
