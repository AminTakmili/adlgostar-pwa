...
import { reportWorkingFriday } from 'src/app/core/models/report.model';
...
@Component({
  selector: 'app-business-report-workinghour-working-friday',
  templateUrl: './business-report-workinghour-working-friday.component.html',
  styleUrls: ['./business-report-workinghour-working-friday.component.scss'],
})
export class BusinessReportWorkinghourWorkingFridayComponent implements OnInit {

  
  @Input('businessId')businessId:string
	columnHighcharts: typeof Highcharts = Highcharts;
	plotShow: boolean = false;
	data: reportWorkingFriday[];
 ...
 setcolumnChart(dataSet: reportWorkingFriday[]) {
		if (this.plotShow) {
			this.plotShow=false
		}
		console.log(dataSet);
...
console.log(dataSetgroupBy);
		...
		for (const year in dataSetgroupBy) {
			for (let index = startMonth; index <= endMonth; index++) {
				...
				if (
					...
					fackData.push(
						new reportWorkingFriday().deserialize({
							business_name: '',
							year,
							month: index,
		...
		const dataworkingOverTimeHourCount = fackData.map((item: reportWorkingFriday) => {
			return Math.round(item.working_friday_hour_count*10)/10 ;
		});
		const dataworkingOverTimePrice = fackData.map((item: reportWorkingFriday) => {
			return Math.round(item.working_friday_price*10)/10 ;
		});
		const dataCategories = fackData.map((item: reportWorkingFriday) => {
    ...
    console.log(dataCategories);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'جمعه کاری':' جمعه کاری سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع جمعه کاری به تفکیک هر ماه را میتوانید مشاهده کنید ' 
	...
	async getData(	filtered_business_id:string=this.businessId,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/workingFridayList', {
			...
			.subscribe(
				async (res: any) => {
					...
					this.loading=false
					console.log(res);
					this.data = res.list.map((item: reportWorkingFriday) => {
						return new reportWorkingFriday().deserialize(item);
					});
