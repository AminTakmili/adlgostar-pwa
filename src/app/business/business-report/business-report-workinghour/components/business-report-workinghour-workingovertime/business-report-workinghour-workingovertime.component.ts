...
@Component({
  selector: 'app-business-report-workinghour-workingovertime',
  templateUrl: './business-report-workinghour-workingovertime.component.html',
  styleUrls: ['./business-report-workinghour-workingovertime.component.scss'],
})
export class BusinessReportWorkinghourWorkingovertimeComponent implements OnInit {
...
console.log(dataSetgroupBy);
    ...
    console.log(dataCategories);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'اضافه کاری':' اضافه کاری سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع اضافه کاری به تفکیک هر ماه را میتوانید مشاهده کنید ' 
	...
	async getData(	filtered_business_id:string=this.businessId,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/workingOverTimeList', {
