...
@Component({
  selector: 'app-business-report-insurance',
  templateUrl: './business-report-insurance.component.html',
  styleUrls: ['./business-report-insurance.component.scss'],
})
export class BusinessReportInsuranceComponent implements OnInit {
  pageTitle:string='گزارشات بیمه'
...
console.log(dataSetgroupBy);
    ...
    console.log(dataCategories);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'بیمه':' بیمه سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع بیمه به تفکیک هر ماه را میتوانید مشاهده کنید ' 
	...
	async getData(	filtered_business_id:string=this.id,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/insuranceList', {
