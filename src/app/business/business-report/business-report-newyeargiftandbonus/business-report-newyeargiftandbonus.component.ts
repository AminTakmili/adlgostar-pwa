...
import { reportNewYearGiftAndBonusList } from 'src/app/core/models/report.model';
...
@Component({
  selector: 'app-business-report-newyeargiftandbonus',
  templateUrl: './business-report-newyeargiftandbonus.component.html',
...
getbusiness(term: string = null): Observable<any> {
	...
	setcolumnChart(dataSet: reportNewYearGiftAndBonusList[]) {
		if (this.plotShow) {
			this.plotShow=false
		}
		console.log(dataSet);
...
console.log(dataSetgroupBy);
		...
		for (const year in dataSetgroupBy) {
			for (let index = 1; index <= 12; index++) {
				...
				if (
					...
					fackData.push(
						new reportNewYearGiftAndBonusList().deserialize({
							business_name: '',
							year,
							month: index,
							amount: 0,
		...
		const bonusDataColumn = fackData.map((item: reportNewYearGiftAndBonusList) => {
			return Math.round(item.bonus_amount*10)/10 ;
		});
		const newYearGiftDataColumn = fackData.map((item: reportNewYearGiftAndBonusList) => {
			return Math.round(item.new_year_gift_amount*10)/10 ;
		});
		const dataCategories = fackData.map((item: reportNewYearGiftAndBonusList) => {
	...
	async getData(	filtered_business_id:string=this.id,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/newYearGiftAndBonusList', {
			...
			.subscribe(
				async (res: any) => {
					...
					this.loading=false
					console.log(res);
					this.data = res.list.map((item: reportNewYearGiftAndBonusList) => {
						return new reportNewYearGiftAndBonusList().deserialize(item);
					});
