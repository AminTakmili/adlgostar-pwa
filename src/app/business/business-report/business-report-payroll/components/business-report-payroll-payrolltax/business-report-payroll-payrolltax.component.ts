...
@Component({
  selector: 'app-business-report-payroll-payrolltax',
  templateUrl: './business-report-payroll-payrolltax.component.html',
  styleUrls: ['./business-report-payroll-payrolltax.component.scss'],
})
export class BusinessReportPayrollPayrolltaxComponent implements OnInit {
...
console.log(dataSetgroupBy);
    ...
    console.log(dataCategories);
		...
		this.columnChartOptions = {
			...
			title: {
				text: Object.keys(dataSetgroupBy).length!=1?'مالیات ها بر حقوق':' مالیات ها بر حقوق سال '+Object.keys(dataSetgroupBy)[0],
			},
			subtitle: {
				text:
					' مجموع مالیات ها بر حقوق هرماه را به تفکیک میتوانید مشاهده کنید ' 
	...
	async getData(	filtered_business_id:string=this.businessId,filtered_from_date:string=this.startDate.get('filtered_from_date').value,filtered_to_date:string=this.endDate.get('filtered_to_date').value) {
		...
		// }
		this.global
			.httpPost('report/payroll/payrollTaxList', {
