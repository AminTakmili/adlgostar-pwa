...
export class ContractDetailComponent implements OnInit {
	...
	async getData(id : string){
		...
		await	this.getSentenceListData(this.route.snapshot.paramMap.get('id'));
	...
	async getSentenceListData(id : string){
      ...
      this.SentenceList= res.list.map((item:sentence,index:number)=>{
	...
	submitConfirm(){
		this.global.showAlert('تایید قرارداد',
		...
		[
			...
			},
			{
				...
				handler:async()=>{
					...
					this.global.httpPost('contract/confirm',{id:this.id}).subscribe(
						async (res:any) => {
							await this.global.dismisLoading()
							console.log(res);
							this.dataList.is_confirmed=1
							
							this.global.showToast(res.msg,500,'top','success','ios')
