...
@Component({
  selector: 'app-recognizance-template-add',
  templateUrl: './recognizance-template-add.component.html',
  styleUrls: ['./recognizance-template-add.component.scss'],
})
export class RecognizanceTemplateAddComponent implements OnInit {

 

	pageTitle: string = "افزودن قالب تعهدنامه جدید";
	...
	getData() {
		...
		this.global.parallelRequest([  contractTemplateVariable])
			.subscribe(([  contractTemplateVariableRes = '' ]) => {
	...
	async onSubmit() {
		...
		if (this.addForm.valid) {
			...
			this.global.httpPost('recognizanceTemplate/add', this.addForm.value)
				.subscribe(async (res:any) => {

					await this.global.dismisLoading();
					// console.log(res:any);
					this.navCtrl.navigateForward('/recognizance/template');
					this.global.showToast('قالب تعهدنامه با نام  ' + this.addForm.value.name + ' ثبت شد .');
					this.addForm.reset();
				}, async (error:any) => {
					await this.global.dismisLoading();
